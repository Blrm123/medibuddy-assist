...
interface DoctorEarningsSummary {
...
interface DoctorPayout {
  ...
  netAmount: number;
  paypalEmail: string;
  status: "PROCESSING" | "PROCESSED" | string;
}

interface DoctorEarningsProps {
  earnings: DoctorEarningsSummary;
  payouts?: DoctorPayout[];
}

export function DoctorEarnings({ earnings, payouts = [] }: DoctorEarningsProps) {
  ...
  const { loading, fn: submitPayoutRequest } = useFetch<
  ...
  const handlePayoutRequest = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    const response = await submitPayoutRequest(formData);
              ...
              <div className="space-y-2">
                ...
                <Alert>
                  ...
                  <AlertDescription className="text-sm">
                    Your payout request is being processed. You will receive the
