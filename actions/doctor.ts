...
export async function setAvailabilitySlots(formData: FormData) {
  ...
  try {
    ...
    // Note: In a real app, you might want to check for overlapping appointments
    // but for now we'll just delete all existing availability slots
...
export async function markAppointmentCompleted(formData: FormData) {
  ...
  try {
    ...
    throw new Error(
      "Failed to mark appointment as completed: " + error
    );
  }
}
